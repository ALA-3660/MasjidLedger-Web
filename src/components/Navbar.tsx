      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="flex items-center space-x-1.5">
                  <h1 className="text-lg font-bold text-slate-900 tracking-tight flex items-center gap-1 font-heading">
                    <span>{language === 'bn' ? 'মসজিদলেজার' : 'MasjidLedger'}</span>
                    <span className="text-blue-600 italic font-black text-sm">Pro</span>
                  </h1>
                  <span className="bg-blue-50 text-blue-700 text-[10px] font-bold px-2 py-0.5 rounded-full border border-blue-200/60 hidden sm:inline-block uppercase">
                    v2.6
                  </span>
                </div>
                <p className="text-[11px] text-slate-500 font-medium hidden sm:block truncate max-w-[200px]">
          ...
          <div className="hidden md:flex items-center space-x-3">
            <div className="flex bg-slate-100 rounded-full px-3.5 py-1.5 items-center space-x-2 border border-slate-200">
            ...
            {currentUser?.role === 'SUPER_ADMIN' && mosquesList.length > 1 && (
              <div className="flex items-center space-x-2 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg text-xs">
                ...
                <span className="text-slate-700 font-medium">{language === 'bn' ? 'মসজিদ:' : 'Mosque:'}</span>
                <select
                  id="select-super-mosque"
                  value={activeMosque?.id || ''}
                  onChange={(e) => onMosqueChange?.(e.target.value)}
                  className="bg-white border border-slate-200 rounded px-2 py-0.5 text-xs text-slate-800 focus:ring-1 focus:ring-blue-500 focus:outline-hidden font-medium"
                >
                  {mosquesList.map((m) => (
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <div className="relative group hidden sm:block">
              <button
                id="btn-quick-action"
                className="flex items-center space-x-1.5 bg-slate-900 hover:bg-slate-800 text-white px-3.5 py-2 rounded-lg text-xs font-semibold shadow-sm transition-colors"
              >
                <PlusCircle className="w-4 h-4 text-blue-400" />
                <span>{language === 'bn' ? '+ দ্রুত এন্ট্রি' : '+ Quick Entry'}</span>
              </button>
              <div className="absolute right-0 mt-1 w-48 bg-white border border-slate-200 rounded-xl shadow-lg py-1.5 hidden group-hover:block z-50 divide-y divide-slate-100">
                <div className="py-1">
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-blue-50 hover:text-blue-700 flex items-center space-x-2.5 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-rose-50 hover:text-rose-700 flex items-center space-x-2.5 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-teal-50 hover:text-teal-700 flex items-center space-x-2.5 transition-colors"
                  >
                    <span className="w-2 h-2 rounded-full bg-teal-500"></span>
                    <span className="font-medium">{t.addDonation}</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              onClick={() => onOpenAi?.()}
              className="flex items-center space-x-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 px-3 py-1.5 rounded-lg text-xs font-semibold shadow-xs transition-all"
              title="AI আর্থিক অডিটর ও সারসংক্ষেপ"
            >
              <Sparkles className="w-3.5 h-3.5 text-blue-600" />
              <span className="hidden md:inline">{language === 'bn' ? 'এআই অডিটর' : 'AI Advisor'}</span>
            </button>
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 text-xs font-medium shadow-xs"
            >
            ...
            <div className="flex items-center space-x-2 pl-2 border-l border-slate-200">
              <div className="w-8 h-8 rounded-full bg-blue-100 border border-blue-200 text-blue-800 font-bold text-xs flex items-center justify-center">
